...
function isAuthenticated(req) {
  ...
  const headerSecret = req.headers["x-checkin-secret"];
  ...
  const session = getCookie(req, "checkin_session");
...
async function supabaseRpc(functionName, body) {
  ...
  if (!response.ok) {
    ...
    const error = new Error(
      detail || "Erro ao executar operação no Supabase."
    );
  ...
  try {
    ...
    if (action !== "ENTRY" && action !== "EXIT") {
      return res.status(400).json({
        ...
        message: "A operação deve ser ENTRY ou EXIT."
      });
    }
    ...
    if (!row) {
      return res.status(500).json({
        success: false,
        result: "EMPTY_RPC_RESPONSE",
        message: "O Supabase não retornou o resultado do check-in."
      });
    }

    const success = row.success === true;

    let httpStatus = success ? 200 : 409;

    if (row.result === "TICKET_NOT_FOUND") {
      httpStatus = 404;
    }

    if (row.result === "PAYMENT_NOT_PAID") {
      httpStatus = 409;
    }
    ...
    return res.status(httpStatus).json({
      ...
      ticket: {
        ticketCode: row.ticket_code || ticketCode,
        fullName: row.full_name || "",
        ticketType: row.ticket_type || "",
        ticketLot: row.ticket_lot || "",
    ...
    return res.status(500).json({
      success: false,
      result: "SERVER_ERROR",
      message: "Erro interno ao processar o check-in."
    });
  }
};
